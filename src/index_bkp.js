...
// Use a free tier model if not specified
const openrouterModelName = process.env.OPENROUTER_MODEL || "gpt-3.5-turbo";
...
// Build OpenRouter model using ChatOpenAI with custom endpoint
const buildOpenRouterModel = () =>
  new ChatOpenAI({
...
const openrouterModel = buildOpenRouterModel();
...
catch (error) {   console.error("Error invoking LLM or processing response:", error);
...
// Use a fixed non-conflicting port to avoid address-in-use errors (ignore .env PORT)
